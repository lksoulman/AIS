...
function MatchesMask(const Str, Mask: string): Boolean;
...
type
  ...
  public
    ...
    function Matches(const Str: string): Boolean;
  end;
      ...
      case P^ of
        '-':
          ...
          begin
            Inc(P);
            {$IFDEF D2009}
            for C := LastChar to UpCase(P^) do Include(CharSet, AnsiChar(C));
            {$ELSE}
...
function MatchesMaskStates(const Str: string;
...
function TMask.Matches(const Str: string): Boolean;
begin
  Result := MatchesMaskStates(Str, Slice(PMaskStateArray(FMask)^, FSize));
end;
