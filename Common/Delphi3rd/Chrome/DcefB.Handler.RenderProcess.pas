...
type
  ...
  //外部函数类   --add by sgao 2017-06-19
  TChromeExternal = class
    //执行外部函数
    class procedure Execute(const AFuncName, AParams: string);
  end;
...
begin
  ...
  //add by sgao 2017-06-10 发送焦点节点改变消息
  LMsg := TCefProcessMessageRef.New(RENDERPROCESS_FOCUSEDNODE_CHANGED_MSG);
  LMsg.ArgumentList.SetSize(2);
  LMsg.ArgumentList.SetString(0, IntToStr(frame.GetIdentifier));
  LIsEditable := Assigned(node) and node.IsEditable;
  LMsg.ArgumentList.SetBool(1, LIsEditable);
  browser.SendProcessMessage(TCefProcessId.PID_BROWSER, LMsg);
end;
...
{$IFDEF CEF_MULTI_THREADED_MESSAGE_LOOP}, True {$ENDIF})
  ...
  if Assigned(FOnWebKitInitialized) then
   ...
   //添加外部函数类  add by sgao  2017-06-19
   DcefBApp.CefRegisterExtension('ChromeExternal',
          TChromeExternal
  {$IFDEF CEF_MULTI_THREADED_MESSAGE_LOOP}, True {$ENDIF})
end;
...
//执行外部函数
class procedure TChromeExternal.Execute(const AFuncName, AParams: string);
var
  LMsg: ICefProcessMessage;
begin
  LMsg := TCefProcessMessageRef.New(RENDERPROCESS_EXTERNAL_FUNC_MSG);
  LMsg.ArgumentList.SetSize(2);
  LMsg.ArgumentList.SetString(0, AFuncName);
  LMsg.ArgumentList.SetString(1, AParams);
