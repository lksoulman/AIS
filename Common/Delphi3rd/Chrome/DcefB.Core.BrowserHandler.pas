...
{$ENDIF}
  Pointer(FEvents) := nil;   //edit by sgao 2017-06-09  FEvents即是TfrmWebPageEx，已在这之前释放，所以把接口的值置为nil，防止_InfClear出错。
  inherited;
end;
...
function TDcefBHandler.OnProcessMessageReceived(const browser: ICefBrowser;
  ...
  procedure DoFocusedNodeChanged;
  var
   PArgs: PFocusedNodeChangedArgs;
   LFrame: ICefFrame;
  begin
    New(PArgs);
    LFrame := browser.GetFrameByident(StrToInt64(message.ArgumentList.GetString(0)));
    PArgs.frame := @LFrame;
    PArgs.isEditable := message.ArgumentList.GetBool(1);
    TDcefBUtils.SendMsg(browser, WM_FocusedNodeChanged, LParam(PArgs));
    Dispose(PArgs);
  end;
  procedure DoExternalFuncExec;
  var
    PArgs: PExternalFuncArgs;
    LFunc, LParams: string;
  begin
    LFunc := message.ArgumentList.GetString(0);
    LParams := message.ArgumentList.GetString(1);
    New(PArgs);
    PArgs.functName := @LFunc;
    PArgs.params := @LParams;
    TDcefBUtils.SendMsg(browser, WM_ExternalFuncExec, LParam(PArgs));
    Dispose(PArgs);
  end;
begin
  ...
  //渲染进程发送的焦点节点改变事件
  else if SameText(message.Name, RENDERPROCESS_FOCUSEDNODE_CHANGED_MSG) then
  begin
    DoFocusedNodeChanged;
    Result := True;
  end
  //渲染进程发送的外部函数执行事件
  else if SameText(message.Name, RENDERPROCESS_EXTERNAL_FUNC_MSG) then
  begin
    DoExternalFuncExec;
    Result := True;
  end
