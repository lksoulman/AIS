...
type
  ...
  IDcefBrowser = interface
    ...
    //焦点节点改变时触发事件(isEditable:焦点节点是否是编辑节点)  -- add by sgao 2017-06-10
    procedure doOnFocusedNodeChanged(const browser: ICefBrowser; const frame: ICefFrame; isEditable: Boolean);
    //外部函数执行时触发事件 --add by sgao 2017-06-19
    procedure doOnExternalFuncExec(const browser: ICefBrowser; const AFuncName, AParams: string);
