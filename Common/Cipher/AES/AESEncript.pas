...
{$ENDIF}
uses
  SysUtils, Classes, Math, ElAES,base64;
...
function EncryptString128B64(Value: AnsiString; Key: AnsiString;
  AESVec: TAESBuffer): AnsiString;
function DecryptString(Value: AnsiString; Key: AnsiString;
  KeyBit: TKeyBit = kb128): AnsiString;
function DecryptString128B64(Value: AnsiString; Key: AnsiString;
  AESVec: TAESBuffer): AnsiString;
...
begin
  ...
  try
    ...
    {$IFDEF VER210}
      ...
      Result := Base64EncodeStr(st);
    {$ELSE}
      Result := Base64EncodeStr(DS.DataString);
    {$ENDIF}
  finally
    SS.Free;
    DS.Free;
...
begin
  ...
  try
    ...
    Move(PAnsiChar(Key)^, AESKey128, Min(SizeOf(AESKey128), Length(Key)));
    //EncryptAESStreamECB(SS, 0, AESKey128, DS);
    EncryptAESStreamCBC(SS,SS.Size - SS.Position,AESKey128,AESVec,DS);
    {$IFDEF VER210}
      SetLength(st,Ds.Size);
      DS.Position := 0;
      DS.ReadBuffer(PAnsiChar(st)^,DS.Size);
...
begin
  Result := '';
  SS := TStringStream.Create(Base64DecodeStr(Value));
  DS := TStringStream.Create('');
  try
    Size := SS.Size;
...
begin
  ...
  try
    ...
    FillChar(AESKey128, SizeOf(AESKey128), 0 );
    Move(PAnsiChar(Key)^, AESKey128, Min(SizeOf(AESKey128), Length(Key)));
    DecryptAESStreamCBC(SS, SS.Size - SS.Position, AESKey128,AESVec, DS);
    Result :=  DS.DataString;
  finally
    SS.Free;
    DS.Free;
