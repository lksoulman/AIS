...
type
  ...
  // 终端配置管理
  TConfigImpl = class(TSyncAsyncImpl, IConfig)
  private
    // 应用程序目录
    FAppPath: string;
    // 基础配置文件
  ...
  protected
    // 初始化web信息
    procedure DoInitWebInfo;
    // 初始化期货信息
    procedure DoInitFutureInfo;
    // 初始化期货信息
    procedure DoInitHqServerTypeInfo;
    // 初始化基础信息
...
begin
  ...
  FFutureInfo := TFutureInfoImpl.Create as IFutureInfo;
  FCompanyInfo := TCompanyInfoImpl.Create as ICompanyInfo;
  FHqServerTypeInfo := THqServerTypeInfoImpl.Create as IHqServerTypeInfo;
  ...
  FFutuesServerInfo := TServerInfoImpl.Create('Futures') as IServerInfo;
...
procedure TConfigImpl.DoInitWebInfo;
var
  LFile: string;
  LNode: TXmlNode;
  LXml: TNativeXml;
  LNodeList: TList;
begin
  LFile := GetCfgPath + 'Web\WebCfg.xml';
...
procedure TConfigImpl.DoInitFutureInfo;
var
  LFile: string;
  LNode: TXmlNode;
  LXml: TNativeXml;
  LNodeList: TList;
begin
  LFile := GetCfgPath + 'HQ\FutureCodeCfg.xml';
  if FileExists(LFile) then begin
    LXml := TNativeXml.Create(nil);
    try
      LXml.LoadFromFile(LFile);
      ...
      LNodeList := TList.Create;
      try
        LNode.FindNodes('FutureCode', LNodeList);
        FFutureInfo.LoadXmlNodes(LNodeList);
      finally
        LNodeList.Free;
      end;
    finally
...
procedure TConfigImpl.DoInitHqServerTypeInfo;
var
  LFile: string;
  LNode: TXmlNode;
  LXml: TNativeXml;
  LNodeList: TList;
begin
  LFile := GetCfgPath + 'HQ\ServerTypeCfg.xml';
  if FileExists(LFile) then begin
    LXml := TNativeXml.Create(nil);
    try
      LXml.LoadFromFile(LFile);
      ...
      LNodeList := TList.Create;
      try
        LNode.FindNodes('ServerType', LNodeList);
        FHqServerTypeInfo.LoadXmlNodes(LNodeList);
      finally
        LNodeList.Free;
      end;
    finally
...
procedure TConfigImpl.SyncBlockExecute;
begin
  FUserInfo.LoadCache;
...
procedure TConfigImpl.AsyncNoBlockExecute;
begin

end;

function TConfigImpl.Dependences: WideString;
begin
...
function TConfigImpl.GetFutureInfo: IFutureInfo;
begin
  Result := FFutureInfo;
end;
...
function TConfigImpl.GetHqServerTypeInfo: IHqServerTypeInfo;
begin
  Result := FHqServerTypeInfo;
end;
