...
type
  ...
  public
    ...
    // 登录类型
    function GetAccountType: TAccountType; safecall;
    // 登录类型
    procedure SetAccountType(AccountType: TAccountType); safecall;
  end;
    ...
    if FAppContext.GetConfig <> nil then begin
      if FAppContext.GetCipherMgr <> nil then begin
        if GetSavePassword then begin
          LUserPassword := (FAppContext.GetCipherMgr as ICipherMgr).AESEncrypt128(AnsiString(FUserPassword));
          LAssetUserPassword := (FAppContext.GetCipherMgr as ICipherMgr).AESEncrypt128(FAssetUserPassword);
        end;
        LBindOrgSign :=  (FAppContext.GetCipherMgr as ICipherMgr).AESEncrypt128(FBindOrgSign);
      end;
        ...
        if LStringList.DelimitedText <> '' then begin
          ...
          if FAppContext.GetCipherMgr <> nil then begin
            if GetSavePassword then begin
              ...
              FAssetUserPassword := (FAppContext.GetCipherMgr as ICipherMgr).AESDecrypt128(AnsiString(LAssetUserPassword));
            end else begin
              FAssetUserPassword := '';
            end;
            FBindOrgSign := (FAppContext.GetCipherMgr as ICipherMgr).AESDecrypt128(AnsiString(FBindOrgSign));
          end else begin
            FAssetUserPassword := '';
            FBindOrgSign := '';
          end;
...
begin
  if (FAppContext <> nil) and (FAppContext.GetCipherMgr <> nil) then begin
    ...
    LCiperPassword := (FAppContext.GetCipherMgr as ICipherMgr).RSAEncryptAndBase64(LCiperPassword);
  end else begin
    LCiperPassword := '';
  end;
...
procedure TUserInfoImpl.SetAccountType(AccountType: TAccountType);
begin
