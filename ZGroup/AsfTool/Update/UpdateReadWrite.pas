...
////////////////////////////////////////////////////////////////////////////////
//
// Description£º Update ReadWrite
// Author£º      lksoulman
// Date£º        2017-10-12
// Comments£º
//
...
procedure TUpdateReadWrite.UnInitialize(AContext: IUpdateAppContext);
begin
...
begin
  ...
  if LNode <> nil then begin
    LNode.Value := AUpdateFileInfo.Name;
  end;
  LNode := ANode.NodeNew('Size');
  if LNode <> nil then begin
    LNode.Value := IntToStr(AUpdateFileInfo.Size);
  end;
  LNode := ANode.NodeNew('CrcValue');
  if LNode <> nil then begin
    LNode.Value := AUpdateFileInfo.CrcValue;
  end;
  LNode := ANode.NodeNew('MD5Value');
  if LNode <> nil then begin
    LNode.Value := AUpdateFileInfo.MD5Value;
  end;
  LNode := ANode.NodeNew('RelativePath');
  if LNode <> nil then begin
    LNode.Value := AUpdateFileInfo.RelativePath;
  end;
  LNode := ANode.NodeNew('UpgradeType');
  if LNode <> nil then begin
    LNode.Value := IntToStr(AUpdateFileInfo.UpgradeTypeToInt(AUpdateFileInfo.UpgradeType));
  end;
end;
