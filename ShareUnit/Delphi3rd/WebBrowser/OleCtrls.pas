...
begin
  DestroyStorage;
  if Assigned(FPersistStream) then
  begin
...
begin
  ...
  if Assigned(FPersistStream) and (FPersistStream.IsDirty <> S_FALSE) then DesignModified;
end;
...
begin
  ...
  Pointer(Result) := Temp.VDispatch;          //强制转换成Pointer再赋值就不会增加引用计数 --edit by sgao
end;
...
begin
  ...
  Pointer(Result) := Temp.VUnknown;           //强制转换成Pointer再赋值就不会增加引用计数 --edit by sgao
...
const
  ExtKeys = [VK_UP, VK_DOWN]; { extended keys }
var
  WinMsg: TMsg;
begin
  ...
  if not (WinMsg.wParam in ExtKeys) then    //当在WebBrowser内触发上下方向键时，不再向下分发消息 Added by sgao 2013-6-3
...
begin
  ...
  begin
    ...
    begin
      ...
      if IsAccelerator(FControlInfo.hAccel, FControlInfo.cAccel, @Msg, Cmd) then
      begin
...
{ TOleControl.IServiceProvider }

function TOleControl.QueryService(const rsid, iid: TGuid; out Obj): HResult;
begin
  if Assigned(FServiceQuery) then Result := FServiceQuery(rsid, iid, Obj)
...
initialization
  GroupDescendentsWith(TOleControl, Controls.TControl);
