...
type
  ...
  private
    ...
    //焦点节点改变消息处理函数 --add by sgao 2017-06-10
    procedure OnFocusedNodeChanged(aBrowser: ICefBrowser; LParam: LParam);
    //外部函数执行消息处理函数 --add by sgao 2017-06-19
    procedure OnExternalFuncExec(aBrowser: ICefBrowser; LParam: LParam);
...
begin
  ...
  //add by sgao 2017-06-10 CancelDefaultEvent取消TBrowserView的默认操作。PArgs.Result取消浏览器的默认弹出操作。
  if PArgs.Result^ then
    Result := S_FALSE
  else
    Result := S_OK;
end;
...
begin
  ...
  begin
    if Pos('.ico', favUrlList[Index]) > 0 then
    begin
...
//焦点节点改变消息处理函数 --add by sgao 2017-06-10
procedure TBrowserView.OnFocusedNodeChanged(aBrowser: ICefBrowser; LParam: LParam);
var
  PArgs: PFocusedNodeChangedArgs;
begin
  PArgs := PFocusedNodeChangedArgs(LParam);
  FEvents.doOnFocusedNodeChanged(aBrowser, PArgs.frame^, PArgs.isEditable);
end;

//外部函数执行消息处理函数 --add by sgao 2017-06-19
procedure TBrowserView.OnExternalFuncExec(aBrowser: ICefBrowser; LParam: LParam);
var
  PArgs: PExternalFuncArgs;
begin
  PArgs := PExternalFuncArgs(LParam);
  FEvents.doOnExternalFuncExec(aBrowser, PArgs.functName^, PArgs.params^);
end;
...
begin
  case Message.Msg of
    ...
    //add by sgao 2017-06-10 增加焦点节点改变消息
    WM_FocusedNodeChanged:
      OnFocusedNodeChanged(GetCefBrowser, Message.LParam);
    //add by sgao 2017-06-10 增加外部函数执行消息
    WM_ExternalFuncExec:
      OnExternalFuncExec(GetCefBrowser, Message.LParam);
    WM_FileDialog:
